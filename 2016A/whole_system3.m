...
for L_sum = 0 : 0.001 : 40;

    h1_theta_test; % 根据遍历到的theta值求出吃水深度h1
    ...
    % R_sum 游动区域半径
    R_sum(time_test) = x(Index) + temp_r_sum;
    
    abs_test(time_test) = abs(y(Index) + temp_h_sum + h1 - h_sum);
    
    if abs(y(Index) + temp_h_sum + h1 - h_sum) < eps
        L_sum
        break;
    end
    
    h_time_test(time_test) = h1;
